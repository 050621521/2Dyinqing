...
除渲染管线外，本项目的编辑器体验、对象模型、蓝图系统、资源管理、运行调试和项目组织应尽量向虚幻引擎的游戏开发工作流靠拢。

这里的“向虚幻靠拢”不是复制虚幻的 3D 渲染能力、材质系统或大型实时渲染管线，而是学习它在制作游戏时的核心组织方式：

- 编辑器结构靠近虚幻：项目、关卡、内容浏览器、细节面板、蓝图、运行预览、布局管理应形成稳定一致的工作流。
- 对象模型靠近虚幻：Actor 是场景对象，组件承载能力，Actor 蓝图定义可复用类，关卡蓝图负责关卡级调度。
- 蓝图能力靠近虚幻：节点应逐步具备类型、变量、事件、流程控制、函数/宏和调试能力，避免只堆临时动作节点。
- 资源系统靠近虚幻：图片、动画、UI、枚举、蓝图类、关卡等都应作为资产被浏览、引用、校验和维护。
- 调试体验靠近虚幻：运行时日志、变量观察、复现录制、节点执行痕迹和游戏预览应服务于定位游戏逻辑问题。

做功能设计时，应优先保证这些方向服务于 2D 游戏制作本身。渲染部分保持 2D、轻量、可控，不追求虚幻式复杂 3D 渲染能力。
